...
def Main():
    ...
    Vars=["climatic_mass_balance","bheatflx","ice_surface_temp"]
    PlotLabels=["surface mass balance","geothermal heatflux", "ice surface temperature"]
    PlotLabels=["SMB","Geo. heatflux", "Surf. temp."]
    for i in range(len(Vars)):
        print(Vars[i])
        PlotFiles=CreateTSInputAnomExt(Vars[i],"Hudson")
        # if i <= 1:
        print(PlotFiles)
        ...
        horizontalalignment='center', verticalalignment='center',
        transform=ax[i][0].transAxes,fontsize=LabelFS)
        PlotFiles=CreateTSInputAnomExt(Vars[i],"Kenzie")
        # if i <= 1:
    ...
    plt.tight_layout()
    # plt.show()
    SavePlot("SIFig03","TSSurges_AnomalyExtended")
