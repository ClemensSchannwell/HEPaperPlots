...
def Main():
    ...
    for i,txt in enumerate(Text):
        if i < 4:
            print(HEFreq_Hudson[i])
            for iCross in range(len(HEFreq_Hudson[i])):
                ax[0].scatter(HEFreq_Hudson[i][iCross],DeltaVol_Hudson[i][iCross],s=50,marker="v",c=Colors[i], edgecolors=Colors[i])
                ax[1].scatter(HEFreq_Kenzie[i][iCross],DeltaVol_Kenzie[i][iCross],s=50,marker="v",c=Colors[i], edgecolors=Colors[i])
            ...
            for iCross in range(len(HEFreq_Hudson[i])):
                ax[0].scatter(HEFreq_Hudson[i][iCross],DeltaVol_Hudson[i][iCross],s=50,marker="^",c=Colors[i], edgecolors=Colors[i])
    ...
    plt.tight_layout()
    # plt.show()
    SavePlot("Fig08","IceVolume_PeriodPlotAnomalyCrosses",600)
